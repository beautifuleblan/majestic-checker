...
def get_site_content(proxy, proxy_type, domain):
    if len(proxy) == 4:
        proxy_formatted = {'http': f'{proxy_type.lower()}://{proxy[2]}:{proxy[3]}@{proxy[0]}:{proxy[1]}',
                           'https': f'{proxy_type.lower()}://{proxy[2]}:{proxy[3]}@{proxy[0]}:{proxy[1]}'}
    else:
        proxy_formatted = {'http': f'{proxy_type.lower()}://{proxy[0]}:{proxy[1]}',
                           'https': f'{proxy_type.lower()}://{proxy[0]}:{proxy[1]}'}
    ...
    formatted_domain = domain.replace('https://', '').replace('http://', '').replace('/', '')
